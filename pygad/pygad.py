...
class GA(utils.parent_selection.ParentSelection,
    ...
    @property
    def mutation_by_replacement(self) -> bool:
        return self._mutation_by_replacement

    @mutation_by_replacement.setter
    def mutation_by_replacement(self, mutation_by_replacement):
        try:
            ...
            if self.mutation_type != "random" and mutation_by_replacement:
                if not self.suppress_warnings:
                    warnings.warn(
                        f"The mutation_by_replacement parameter is set to True while the mutation_type parameter is not set to random but ({self.mutation_type}). Note that the mutation_by_replacement parameter has an effect only when mutation_type='random'.")

            self._mutation_by_replacement = mutation_by_replacement
        except Exception as ex:
            self.logger.error(str(ex))
            exit(-1)
    ...
    def allow_duplicate_genes(self) -> bool:
        return self._allow_duplicate_genes
    ...
    def allow_duplicate_genes(self, allow_duplicate_genes):
        ...
        self._allow_duplicate_genes = allow_duplicate_genes
    ...
    @gene_space.setter
    def gene_space(self, gene_space):
        self._gene_space_nested = False
    ...
    def random_mutation_min_val(self):
        return self._random_mutation_min_val

    @random_mutation_min_val.setter
    def random_mutation_min_val(self, random_mutation_min_val):
        # Validate random_mutation_min_val
        if type(random_mutation_min_val) in GA.supported_int_float_types:
            self._random_mutation_min_val = random_mutation_min_val
        else:
            raise TypeError(
                f"The expected type of the 'random_mutation_max_val' parameter is numeric but {type(random_mutation_min_val)} found.")

    @property
    def random_mutation_max_val(self):
        return self._random_mutation_max_val

    @random_mutation_max_val.setter
    def random_mutation_max_val(self, random_mutation_max_val):
        # Validate random_mutation_max_val
        if type(random_mutation_max_val) in GA.supported_int_float_types:
            self._random_mutation_max_val = random_mutation_max_val
        else:
            raise TypeError(
                f"The expected type of the 'random_mutation_max_val' parameter is numeric but {type(random_mutation_max_val)} found.")

    @property
    def gene_type_single(self) -> bool:
        return self._gene_type_single

    @gene_type_single.setter
    def gene_type_single(self, gene_type_single):
        self._gene_type_single = gene_type_single
    ...
    def num_genes(self, num_genes):
        ...
        if self._gene_space_nested:
            if len(self.gene_space) != self.num_genes:
                raise ValueError(
                    f"When the parameter 'gene_space' is nested, then its length must be equal to the value passed to the 'num_genes' parameter. Instead, length of gene_space ({len(self.gene_space)}) != num_genes ({self.num_genes})")

        self._num_genes = num_genes

    @property
    def sol_per_pop(self) -> int:
        return self._sol_per_pop

    @sol_per_pop.setter
    def sol_per_pop(self, sol_per_pop):
        if sol_per_pop <= 0:
            raise ValueError(
    ...
    def initial_population(self, initial_population):
        # Build the initial population
        if initial_population is None:
            if (self.sol_per_pop is None) or (self.num_genes is None):
            ...
            self._initial_population = self.initialize_population(self.init_range_low,
                                                                  self.init_range_high,
                                                                  self.allow_duplicate_genes,
                                                                  True,
                                                                  self.gene_type)
            ...
            if self.gene_type_single == True:
                if self.gene_type[1] == None:
                    self._initial_population = numpy.array(initial_population,
                                                           dtype=self.gene_type[0])
                else:
                    # This block is reached only for non-integer data types (i.e. float).
                    self._initial_population = numpy.round(numpy.array(initial_population,
                                                                       dtype=self.gene_type[0]),
                                                           self.gene_type[1])
            else:
                initial_population = numpy.array(initial_population)
                self._initial_population = numpy.zeros(shape=(initial_population.shape[0],
                                                              initial_population.shape[1]),
                                                       dtype=object)
                for gene_idx in range(initial_population.shape[1]):
                    if self.gene_type[gene_idx][1] is None:
                        self._initial_population[:, gene_idx] = numpy.asarray(initial_population[:, gene_idx],
                                                                              dtype=self.gene_type[gene_idx][0])
                    else:
                        # This block is reached only for non-integer data types (i.e. float).
                        self._initial_population[:, gene_idx] = numpy.round(
            ...
            if self.allow_duplicate_genes == False:
                for initial_solution_idx, initial_solution in enumerate(self.initial_population):
                    if self.gene_space is None:
                        self._initial_population[initial_solution_idx], _, _ = \
                            self.solve_duplicate_genes_randomly(solution=initial_solution,
                        ...
                        self._initial_population[initial_solution_idx], _, _ = \
                            self.solve_duplicate_genes_by_space(solution=initial_solution,
                                                                gene_type=self.gene_type,
                                                                num_trials=10)
        ...
        self._initial_population = self.round_genes(self.initial_population)
    ...
    def gene_type(self, gene_type):
        ...
        if gene_type in GA.supported_int_float_types:
            self._gene_type = [gene_type, None]
        ...
        # A single data type of int with precision.
            ...
            if not len(gene_type) == len(self.initial_population[0]):
                raise ValueError(
                    f"When the parameter 'gene_type' is nested, then it can be either [float, int<precision>] or with length equal to the number of genes parameter. Instead, value {gene_type} with len(gene_type) ({len(gene_type)}) != number of genes ({len(self.initial_population[0])}) found.")
            elif not len(gene_type) == self.num_genes:
                raise ValueError(
                    f"When the parameter 'gene_type' is nested, then it can be either [float, int<precision>] or with length equal to the value passed to the 'num_genes' parameter. Instead, value {gene_type} with len(gene_type) ({len(gene_type)}) != len(num_genes) ({self.num_genes}) found.")
    ...
    def num_parents_mating(self):
        return self._num_parents_mating
    ...
    def crossover_type(self, crossover_type):
        ...
        if (crossover_type is None):
            ...
            if (crossover_type == "single_point"):
                self._crossover_type = self.single_point_crossover
            elif (crossover_type == "two_points"):
                self._crossover_type = self.two_points_crossover
            elif (crossover_type == "uniform"):
                self._crossover_type = self.uniform_crossover
            elif (crossover_type == "scattered"):
                self._crossover_type = self.scattered_crossover
            else:
                raise TypeError(
    ...
    def mutation_type(self, mutation_type):
        ...
        if mutation_type is None:
            ...
            if (mutation_type == "random"):
                self._mutation_type = self.random_mutation
            elif (mutation_type == "swap"):
                self._mutation_type = self.swap_mutation
            elif (mutation_type == "scramble"):
                self._mutation_type = self.scramble_mutation
            elif (mutation_type == "inversion"):
                self._mutation_type = self.inversion_mutation
            elif (mutation_type == "adaptive"):
                self._mutation_type = self.adaptive_mutation
            else:
                raise TypeError(
    ...
    def mutation_num_genes(self):
        return self._mutation_num_genes
        ...
        if not (self.mutation_type is None):
            if mutation_num_genes is None:
                ...
                if (self.mutation_type != "adaptive"):
                    ...
                    if self.mutation_percent_genes == 'default'.lower():
                        ...
                        self._mutation_num_genes = numpy.uint32(
                            (self.mutation_percent_genes*self.num_genes)/100)
                        ...
                        if mutation_num_genes == 0:
                            ...
                            self._mutation_num_genes = 1

                    elif type(self.mutation_percent_genes) in GA.supported_int_float_types:
                        if (self.mutation_percent_genes <= 0 or self.mutation_percent_genes > 100):
                            self.logger.error(
                    ...
                    if type(self.mutation_percent_genes) in [list, tuple, numpy.ndarray]:
                        if len(self.mutation_percent_genes) == 2:
                            self._mutation_num_genes = numpy.zeros_like(
                                self.mutation_percent_genes, dtype=numpy.uint32)
                            for idx, el in enumerate(self.mutation_percent_genes):
                                if type(el) in GA.supported_int_float_types:
                                    if (el <= 0 or el > 100):
                                        raise ValueError(
                                ...
                                # Based on the mutation percentage in the 'mutation_percent_genes' parameter, the number of genes to mutate is calculated.
                                self.mutation_num_genes[idx] = numpy.uint32(
                                    (self.mutation_percent_genes[idx]*self.num_genes)/100)
                                # Based on the mutation percentage of genes, if the number of selected genes for mutation is less than the least possible value which is 1, then the number will be set to 1.
    ...
    def parent_selection_type(self):
        return self._parent_selection_type
    ...
    def parent_selection_type(self, parent_selection_type):
        ...
        if inspect.ismethod(parent_selection_type):
            ...
            if (parent_selection_type == "sss"):
                self._parent_selection_type = self.steady_state_selection
            elif (parent_selection_type == "rws"):
                self._parent_selection_type = self.roulette_wheel_selection
            elif (parent_selection_type == "sus"):
                self._parent_selection_type = self.stochastic_universal_selection
            elif (parent_selection_type == "random"):
                self._parent_selection_type = self.random_selection
            elif (parent_selection_type == "tournament"):
                self._parent_selection_type = self.tournament_selection
            elif (parent_selection_type == "rank"):
                self._parent_selection_type = self.rank_selection
            else:
    ...
    def K_tournament(self, K_tournament):
     # For tournament selection, validate the K value.
        ...
        self._K_tournament = K_tournament

    @property
    def keep_parents(self):
        return self._keep_parents
    ...
    def keep_elitism(self, keep_elitism):
        ...
        self._keep_elitism = keep_elitism
    ...
    @fitness_func.setter
    def fitness_func(self, fitness_func):
    ...
    @property
    def fitness_batch_size(self):
        return self._fitness_batch_size

    @fitness_batch_size.setter
    def fitness_batch_size(self, fitness_batch_size):
        ...
        self._fitness_batch_size = fitness_batch_size
    ...
    @property
    def on_fitness(self):
        return self._on_fitness

    @on_fitness.setter
    def on_fitness(self, on_fitness):
        ...
        if not (on_fitness is None):
            ...
            self._on_fitness = None

    @property
    def on_parents(self):
        return self._on_parents

    @on_parents.setter
    def on_parents(self, on_parents):
        ...
        if not (on_parents is None):
            ...
            self._on_parents = None

    @property
    def on_crossover(self):
        return self._on_crossover

    @on_crossover.setter
    def on_crossover(self, on_crossover):
        ...
        if not (on_crossover is None):
            ...
            if inspect.ismethod(on_crossover):
                ...
                if (on_crossover.__code__.co_argcount == 3):
                    self._on_crossover = on_crossover
                else:
                    raise ValueError(
            ...
            self._on_crossover = None

    @property
    def on_mutation(self):
        return self._on_mutation

    @on_mutation.setter
    def on_mutation(self, on_mutation):
    ...
    @property
    def delay_after_gen(self):
        return self._delay_after_gen

    @delay_after_gen.setter
    def delay_after_gen(self, delay_after_gen):
        # Validate delay_after_gen
        if type(delay_after_gen) in GA.supported_int_float_types:
            if delay_after_gen >= 0.0:
                self._delay_after_gen = delay_after_gen
            else:
                raise ValueError(
    ...
    def save_best_solutions(self):
        return self._save_best_solutions

    @save_best_solutions.setter
    def save_best_solutions(self, save_best_solutions):
        # Validate save_best_solutions
        if type(save_best_solutions) is bool:
            self._save_best_solutions = save_best_solutions
    ...
    def save_solutions(self):
        return self._save_solutions

    @save_solutions.setter
    def save_solutions(self, save_solutions):
        # Validate save_solutions
        if type(save_solutions) is bool:
            self._save_solutions = save_solutions
            if self.save_best_solutions == True:
                if not self.suppress_warnings:
                    warnings.warn(
    ...
    def stop_criteria(self, stop_criteria):
        ...
        if stop_criteria is None:
            ...
            if len(criterion) == 2:
                ...
                self._stop_criteria.append([stop_word, number])
            ...
            self._stop_criteria = list(set(stop_criteria))
    ...
    def parallel_processing(self, parallel_processing):
        if parallel_processing is None:
            self._parallel_processing = None
        elif type(parallel_processing) in GA.supported_int_types:
            if parallel_processing > 0:
                self._parallel_processing = ["thread", parallel_processing]
            else:
    ...
    def __init__(self,
        ...
        try:
            ...
            if self.random_mutation_min_val == self.random_mutation_max_val:
                if not self.suppress_warnings:
                    warnings.warn(
            ...
            self.save_best_solutions = save_best_solutions
            # Holds the best solution in each generation.
            self.best_solutions = []
                        ...
                        if self.gene_space[gene_idx] is None:
                            ...
                            if 'step' in self.gene_space[gene_idx].keys():
                                self.population[sol_idx, gene_idx] = numpy.asarray(
                                    numpy.random.choice(
                                        numpy.arange(start=self.gene_space[gene_idx]['low'],
                                                     stop=self.gene_space[gene_idx]['high'],
                                                     step=self.gene_space[gene_idx]['step']), size=1),
                                    dtype=self.gene_type[0])[0]
                            else:
                                self.population[sol_idx, gene_idx] = numpy.asarray(
                                    numpy.random.uniform(low=self.gene_space[gene_idx]['low'],
        ...
        for generation in range(generation_first_idx, generation_last_idx):
            ...
            if callable(self.parent_selection_type):
                self.last_generation_parents, self.last_generation_parents_indices = \
                    self.parent_selection_type(self.last_generation_fitness,
            ...
            if self.crossover_type is None:
                ...
                if callable(self.crossover_type):
                    self.last_generation_offspring_crossover = self.crossover_type(self.last_generation_parents,
                                                                                   (self.num_offspring,
                                                                                    self.num_genes))
            ...
            if self.mutation_type is None:
                ...
                if callable(self.mutation_type):
                    self.last_generation_offspring_mutation = self.mutation_type(
